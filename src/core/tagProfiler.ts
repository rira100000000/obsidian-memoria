...
export class TagProfiler {
  ...
  private async updateTagProfileForTag(
    ...
    // memory_type の設定
    const hasSemantic = !!(parsedLlmData.body_semantic && parsedLlmData.body_semantic !== '情報なし');
    tpnFrontmatter.memory_type = { semantic: hasSemantic, episodic: true };
    ...
    // 意味記憶セクション
    if (hasSemantic) {
      tpnBodyContent += `## What it is（意味記憶）\n\n${parsedLlmData.body_semantic}\n\n`;
    }

    // エピソード記憶セクション
    tpnBodyContent += `## What it means to us（エピソード記憶）\n\n`;
  ...
  private buildLlmPromptForTagProfiling(
    ...
    let existingSemanticString = "";

    if (existingTpnContent) {
        // 意味記憶セクションの抽出
        const semanticMatch = existingTpnContent.match(/## What it is（意味記憶）\s*([\s\S]*?)(?=\n## What it means to us|$)/);
        if (semanticMatch && semanticMatch[1]) {
            existingSemanticString = semanticMatch[1].trim();
        }

        // エピソード記憶セクション内のサブセクションを抽出（### ヘッダー）
...
TPNは「意味記憶（What it is）」と「エピソード記憶（What it means to us）」の2層構造で構成されます。
...
3.  **解析済みの既存TPN本文データ (もしあれば、更新リスト作成の参考に):**
    * 既存の "body_semantic"（意味記憶）:
        \`\`\`
        ${existingSemanticString || 'なし'}
        \`\`\`
...
**意味記憶（What it is）について:**
タグ「${tagName}」の客観的な定義を "body_semantic" フィールドに記述してください。これはあなたの主観的な所感ではなく、このタグが何であるかの事実的な説明です。
- 一般的に知られた概念（プログラミング言語、ツール、地名等）であれば、あなたの知識から簡潔な定義を書いてください。
- ユーザーが情報源ノートの中でこのタグについて定義や説明をしている場合は、その説明を優先的に使ってください。
- 情報源ノートの frontmatter に semantic_definitions があり、このタグに対応する定義が含まれている場合は、その定義を使ってください。
- 知名度の低い固有名詞で、ユーザーからの説明もない場合は「情報なし」と記述してください。無理に推測しないでください。
- 既にTPNに意味記憶セクションが存在する場合、新しい情報で更新・補完してください。ただし、事実が変わっていない限り既存の定義を維持してください。
...
\`\`\`json
{
  ...
  "body_semantic": "<このタグの客観的な定義・説明。あなたのキャラクターの主観ではなく、一般的に認知されている事実を記述する。ユーザーが会話中に定義を説明した場合はその内容を優先する。知名度の低い概念でユーザーからの説明もない場合は「情報なし」と記述する>",
...
* **意味記憶の安定性:** 'body_semantic' は客観的な定義です。事実が変わっていない限り既存の定義を維持してください。ユーザーが新たに説明を追加した場合のみ更新・補完してください。
