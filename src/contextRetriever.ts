...
import { TagScores, TagProfilingNoteFrontmatter, SummaryNoteFrontmatter, RetrievedContextItem, RetrievedContext, LlmContextEvaluationResponse, ProcessingCallbacks } from './types';
...
export class ContextRetriever {
  ...
  public async retrieveContextForPrompt(userPrompt: string, llmRoleName: string, chatHistory: BaseMessage[], callbacks?: ProcessingCallbacks): Promise<RetrievedContext> {
    ...
    // キーワード抽出とセマンティック検索を並列実行
    callbacks?.onProgress?.('キーワードを抽出しています...');
    callbacks?.onDebugLog?.('記憶検索', 'キーワード抽出とセマンティック検索を開始');

    const tagScores = await this.loadTagScores();
    const existingTags = Object.keys(tagScores);
    callbacks?.onDebugLog?.('記憶検索', `既存タグ数: ${existingTags.length}`);

    const keywordPromise = this.extractAndScoreKeywords(userPrompt, llmRoleName, existingTags);
    const semanticPromise = this.semanticSearch(userPrompt);

    const [keywords, semanticItems] = await Promise.all([keywordPromise, semanticPromise]);
    result.identifiedKeywords = keywords;
    callbacks?.onDebugLog?.('キーワード', `抽出されたキーワード: ${keywords.map(k => `${k.keyword}(${k.inPromptScore})`).join(', ') || 'なし'}`);

    const currentContextItems: RetrievedContextItem[] = [];

    if (result.identifiedKeywords.length > 0) {
      callbacks?.onProgress?.('記憶タグを検索しています...');
      const rankedTags = await this.getRankedTags(result.identifiedKeywords);
      if (rankedTags.length > 0) {
        const maxTags = this.settings.maxTagsToRetrieve !== undefined ? this.settings.maxTagsToRetrieve : 5;
        const selectedTags = rankedTags.slice(0, maxTags);
        callbacks?.onDebugLog?.('タグ検索', `読み込むタグ: ${selectedTags.map(t => `${t.tagName}(${t.score.toFixed(1)})`).join(', ')}`);
        const tpnItems = await this.fetchTpnItems(selectedTags);
        callbacks?.onDebugLog?.('タグ検索', `TPN取得完了: ${tpnItems.length}件`);
        currentContextItems.push(...tpnItems);
      }
    }

    // セマンティック検索結果をマージ（重複排除）
    if (semanticItems.length > 0) {
      callbacks?.onDebugLog?.('セマンティック検索', `セマンティック検索結果: ${semanticItems.length}件`);
      const merged = this.deduplicateItems([...currentContextItems, ...semanticItems]);
      currentContextItems.length = 0;
      currentContextItems.push(...merged);
    }
    ...
    while (evaluationCount < MAX_EVALUATIONS) {
        if (currentContextItems.length === 0 && evaluationCount === 0) {
            ...
            callbacks?.onDebugLog?.('コンテキスト評価', 'TPN情報なし。評価スキップ');
            break;
        }

        callbacks?.onProgress?.('コンテキストを評価しています...');
        callbacks?.onDebugLog?.('コンテキスト評価', `評価ラウンド ${evaluationCount + 1}/${MAX_EVALUATIONS} 開始`);
        ...
        try {
            ...
            if (parsedEval?.sufficient_for_response) {
                ...
                callbacks?.onDebugLog?.('コンテキスト評価', '十分な記憶情報を取得完了');
                break;
            }
            ...
            if (evaluationCount === 0 && parsedEval?.next_summary_notes_to_fetch && parsedEval.next_summary_notes_to_fetch.length > 0) {
                ...
                callbacks?.onProgress?.('サマリーノートを読み込んでいます...');
                callbacks?.onDebugLog?.('追加読み込み', `SN読み込み: ${parsedEval.next_summary_notes_to_fetch.join(', ')}`);
                ...
                callbacks?.onProgress?.('会話ログを読み込んでいます...');
                callbacks?.onDebugLog?.('追加読み込み', `フルログ読み込み: ${parsedEval.requires_full_log_for_summary_note}`);
...
`タスク:
1. ユーザーメッセージと最も関連性の高いタグを既存タグリストから最大5つまで選択してください。
2. 選択・抽出した各タグ/キーワードに対して、今回のユーザーメッセージ内での相対的な重要度を0から100の範囲でスコアリングしてください。
...
ユーザーメッセージは「{userPrompt}」です。このメッセージはLLMキャラクター「{llmRoleName}」に向けられています。
既存のタグリストは以下です
{existingTagsString}
`;
  ...
  private buildLlmContextEvaluationPrompt(currentContextForEval: string, userPrompt: string, llmRoleName: string, currentLevel: 'TPN' | 'SN'): string {
    const promptTemplate = this.settings.prompts?.contextEvaluationPromptBase ||
        `あなたは会話AI「{llmRoleName}」の記憶と思考を補助するAIです。
あなたのタスクは、以下で与えられる参考情報がユーザーの現在の質問に適切に応答するために十分かどうかを評価することです。
応答は必ず以下のJSON形式で出力してください。
\`\`\`json
{
  "sufficient_for_response": <true または false>,
  "reasoning": "<判断理由を簡潔に記述。読み込むべきファイルがわからない場合その理由を記述。>",
  ...
  "requires_full_log_for_summary_note": "<もし 'sufficient_for_response' が false で、特定のサマリーノートのフルログが必要な場合、そのフルログのファイル名を指定 (例: 'YYYYMMDDHHMMSS')。不要なら null>"
}
\`\`\`
考慮事項:
...
- \`next_summary_notes_to_fetch\` と \`requires_full_log_for_summary_note\` は、\`sufficient_for_response\` に用いるファイル名は、以下の参考情報から抽出してください。
---
...
`;
    ...
    try {
      ...
      for (const { entry, similarity } of results) {
        if (entry.sourceType === 'TPN') {
          const tpnItems = await this.fetchTpnItemsFromSemantic(entry.filePath, similarity);
          items.push(...tpnItems);
        } else if (entry.sourceType === 'SN') {
          const snItems = await this.fetchSnItemsFromSemantic(entry.filePath, similarity);
          items.push(...snItems);
        }
      }
  ...
  private async fetchTpnItemsFromSemantic(filePath: string, similarity: number): Promise<RetrievedContextItem[]> {
  ...
  private async fetchSnItemsFromSemantic(filePath: string, similarity: number): Promise<RetrievedContextItem[]> {
    const fileContent = await this.getFileContent(filePath);
    if (!fileContent) return [];
